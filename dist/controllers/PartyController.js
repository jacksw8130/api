                                ...
                                if (bid['yes_or_no'] == "yes") {
                                    if (party_winning_seats >= bid['seat']) {
                                        ...
                                        if (walletTransaction) {
                                            yield User_1.default.findOneAndUpdate({ _id: bid['user_id'] }, { $inc: { wallet: total_amount } }, { new: true, useFindAndModify: false });
                                        }
                                    }
                                    else {
                    ...
                    if (party_winning_seats) {
                        ...
                        for (const ticket of tickets) {
                            ...
                            for (const user_id of user_id_array) {
                                ...
                                let user_data = yield User_1.default.findOne({ _id: user_id });
                                const bids = yield Bid_1.default.find({ ticket_type: "tickets", ticket_id: ticket['_id'], user_id: user_id, result_declare_status: false, bid_status: "pending" });
                                for (const bid of bids) {
                                    // update yes_seats
                                    if (bid['yes_or_no'] == "yes") {
                                        if (party_winning_seats >= bid['seat']) {
                                            ...
                                            // update bid
                                            yield Bid_1.default.findOneAndUpdate({ _id: bid['_id'] }, { result: true, result_declare_status: true, winning_amount: win_amount, bid_status: "win" }, { new: true, useFindAndModify: false });
                                        }
                                        else {
                                            // loss amount
                                            ...
                                            // update bid
                                            yield Bid_1.default.findOneAndUpdate({ _id: bid['_id'] }, { result: false, result_declare_status: true, winning_amount: -loss_amount, bid_status: "loss" }, { new: true, useFindAndModify: false });
                                        }
                                    }
                                    else {
                                        if (party_winning_seats < bid['seat']) {
                                            ...
                                            winning_amount -= loss_amount;
                                            // update bid
                                            yield Bid_1.default.findOneAndUpdate({ _id: bid['_id'] }, { result: true, result_declare_status: true, winning_amount: -loss_amount, bid_status: "loss" }, { new: true, useFindAndModify: false });
                                        }
                                    }
                                }
                                // create transaction
                                ...
                                let walletTransaction = yield new WalletTransaction_1.default(idata).save();
                                if (walletTransaction) {
                                    yield User_1.default.findOneAndUpdate({ _id: user_id }, { $inc: { balance: winning_amount } }, { new: true, useFindAndModify: false });
                                }
                            }
                        }
                    }
